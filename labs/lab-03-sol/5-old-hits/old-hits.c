...
int secret;
const char key[] = "<2DaTM`q8w9C@~1}t{:M#<#HV^SayYT9";
const char  iv[] = "V-To)F`z z'> ;m<";

/*
 * Encrypt and decrypt process taken from
 * https://wiki.openssl.org/index.php/EVP_Symmetric_Encryption_and_Decryption
 */
int proc_encrypt(unsigned char *text, int text_len, const char *key,
		 const char *iv, unsigned char *result, bool encrypt)
{
	EVP_CIPHER_CTX *ctx;
...
int encrypt_links(char enc_b64[], int enc_b64_len)
{
	...
	enc_len = proc_encrypt(links, sizeof(links), key, iv, enc, true);
	if (enc_len == -1) {
...
int show_link(int guess)
{
	...
	enc_len = EVP_DecodeBlock(enc, enc_b64, strlen(enc_b64));
	if (enc_len == -1) {
	...
	dec_len = proc_encrypt(enc, enc_len, key, iv, dec, false);
	if (dec_len == -1) {
